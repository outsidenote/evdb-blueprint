...
# ──────────────────────────────────────────────────────────────────────
# MCP descriptors — emit per-slice tool definitions + per-context aggregator.
#
# Rule: any slice with an HTTP route (REST endpoint) gets a command tool;
# any projection slice gets a query tool. Description is left as the
# @DESCRIPTION_TODO sentinel — implement_slice.py's AI fill replaces it.
# ──────────────────────────────────────────────────────────────────────

def _snake(s: str) -> str:
    """PascalCase → snake_case (uses kebab_case then swaps separator)."""
    return kebab_case(s).replace("-", "_")


def _humanize_pascal(s: str) -> str:
    """PascalCase → human-readable: 'ApproveWithdrawal' → 'Approve Withdrawal'."""
    return re.sub(r"(?<!^)(?=[A-Z])", " ", s)


def _clean_slice_title(raw: dict, slice_name: str) -> str:
    """Extract a clean human-readable slice title.

    Strips the 'slice: ' prefix that config.json puts on every slice's
    `title` field. Falls back to a humanized PascalCase rendering of
    the slice name when no title is present.
    """
    raw_title = raw.get("title", "").strip()
    if raw_title.lower().startswith("slice:"):
        raw_title = raw_title.split(":", 1)[1].strip()
    return raw_title or _humanize_pascal(slice_name)


def _mcp_input_props(fields: list[dict]) -> tuple[str, str]:
    """Build inputSchema properties + required list from command fields.

    Skips generated:true fields (server-set). Returns (props_block, required_block).
    """
    ...
    for f in fields:
        ...
        props_lines.append(f"      {name}: {{ {', '.join(parts)} }},")
        required_names.append(f'"{name}"')
    return "\n".join(props_lines), ", ".join(required_names)


def gen_mcp_command_descriptor(ds: DerivedSlice) -> str:
    """Generate endpoints/<Slice>/MCP/index.ts — write tool descriptor.

    Sentinel description is filled by implement_slice.py's AI step.
    Verified by .ci/stages/mcp_checks.py at CI time.
    """
    sn = ds.slice_name
    sn_camel = ds.slice_name_camel
    context = ds.context
    ...
    # fall back to inserting spaces in the PascalCase slice name.
    cmd_title = (ds.command.get("title") if isinstance(ds.command, dict) else None) \
                or _humanize_pascal(sn)
    cmd_title = cmd_title.replace('"', '\\"')
    slice_title = _clean_slice_title(ds.raw, sn).replace('"', '\\"')

    props_block, required_block = _mcp_input_props(ds.command_fields)
    emit_titles = [f'"{e.get("title", "").replace(chr(34), chr(92) + chr(34))}"'
                   for e in ds.events if e.get("title")]
    emits_block = ", ".join(emit_titles)

    return f"""import {{ defineCommandMcpTool }} from "#abstractions/mcp/defineMcpTool.js";

// @generated by evdb-scaffold — description is filled in by implement_slice.py's AI step.
export const {sn_camel}McpTool = defineCommandMcpTool({{
  name: "{tool_name}",
  title: "{cmd_title}",
...
def gen_mcp_query_descriptor(ds: DerivedSlice) -> str:
    """Generate slices/<Projection>/mcp.ts — read tool descriptor.

    `projectionName` matches the slice name (which is what the projection's
    own index.ts sets as projectionName per the scaffold convention).
    """
    sn = ds.slice_name
    sn_camel = ds.slice_name_camel
    context = ds.context
    ...
    return f"""import {{ defineProjectionMcpTool }} from "#abstractions/mcp/defineMcpTool.js";

// @generated by evdb-scaffold — description is filled in by implement_slice.py's AI step.
export const {sn_camel}McpTool = defineProjectionMcpTool({{
  name: "{tool_name}",
  title: "{tool_title}",
  description: "@DESCRIPTION_TODO",
  context: "{context}",
...
def update_context_mcp_tools(
    ...
    Mirrors `update_context_projections` and `update_routes`: collects all
    ...
    if not content:
        ctx_camel = camel_case(slice_data["context"])
        return f"""// MCP tool exports — collected for discovery at startup.
...
export const {ctx_camel}McpTools: readonly McpToolDescriptor[] = [
  {export_name},
];
"""
...
def scaffold_slice(root: Path, folder: str, dry_run: bool = False) -> dict:
    ...
    if is_projection:
        ...
        # MCP query tool descriptor (read tool) + register in mcpTools.ts
        mcp_query_path = projection_dir / "mcp.ts"
        if not mcp_query_path.exists():
            write_file(mcp_query_path, gen_mcp_query_descriptor(ds))
