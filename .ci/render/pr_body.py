...
Outputs markdown to stdout. Includes:
  ...
  - MCP tool descriptions (if any descriptor files in the workspace)
...
def _extract_mcp_tool(text: str) -> tuple[str, str] | None:
    """Pull (name, description) from a single MCP descriptor file.

    Tool description is at the top level of the descriptor object, which by
    convention sits before `inputSchema:` (where field-level descriptions
    would otherwise be the first match). For projection descriptors there's
    no inline inputSchema — fall back to scanning the whole file.
    """
    boundary = text.find("inputSchema")
    head = text[:boundary] if boundary >= 0 else text

    n = _MCP_NAME_RE.search(head)
    d = _MCP_DESC_RE.search(head)
    if not n or not d:
        return None

    description = next((g for g in d.groups() if g is not None), "")
    return n.group(1), description.strip()


def mcp_descriptions_section() -> str:
    """Render a markdown table of every MCP tool's name + description.

    Reads descriptor files directly from the workspace. Empty when no MCP
    descriptors exist (additive — pre-MCP slices render nothing). Hidden
    sentinels (`@DESCRIPTION_TODO`) are surfaced explicitly so the reviewer
    can spot AI-fill failures even if verify somehow let them through.
    """
    bc_root = Path("src/BusinessCapabilities")
    if not bc_root.exists():
        return ""

    paths = sorted(
        list(bc_root.glob("*/endpoints/*/MCP/index.ts"))
        + list(bc_root.glob("*/slices/*/mcp.ts"))
    )

    rows: list[tuple[str, str, bool]] = []  # (name, description, is_unfilled)
    for p in paths:
        try:
            text = p.read_text()
        except OSError:
            continue
        extracted = _extract_mcp_tool(text)
        if not extracted:
            continue
        name, description = extracted
        is_unfilled = "@DESCRIPTION_TODO" in description
        # Collapse whitespace for the table cell
        description = re.sub(r"\s+", " ", description).strip()
        if len(description) > 240:
            description = description[:237] + "..."
        rows.append((name, description, is_unfilled))
    ...
    lines = [
        "## MCP Tool Descriptions",
        "",
        "AI-generated descriptions for tools the MCP gateway exposes. "
        "**Reviewer:** scan for accuracy — these drive how Claude / OpenAI "
        "agents pick this tool.",
        "",
        "| Tool | Description |",
        "|------|-------------|",
    ]
    for name, desc, unfilled in rows:
        safe = desc.replace("|", "\\|")
        marker = " \U000026a0\U0000fe0f **unfilled**" if unfilled else ""
        lines.append(f"| `{name}` | {safe}{marker} |")
