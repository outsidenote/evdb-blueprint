...
ProjectionSliceTester.run(accountBalanceReadModelSlice, [
  {
    ...
    run: () => {
      ...
      return {
        ...
        then: [{ key: `${account}:USD`, expectedState: { account, balance: 100, currency: "USD" } }],
      };
    },
  },
  {
    ...
    run: () => {
      ...
      return {
        ...
        then: [{ key: `${account}:USD`, expectedState: { account, balance: 150, currency: "USD" } }],
      };
    },
  },
  {
    ...
    run: () => {
      ...
      return {
        ...
        then: [{ key: `${account}:USD`, expectedState: { account, balance: 145, currency: "USD" } }],
      };
    },
  },
  {
    ...
    run: () => {
      ...
      return {
        ...
        then: [
          { key: `${account1}:USD`, expectedState: { account: account1, balance: -110, currency: "USD" } },
          { key: `${account2}:USD`, expectedState: { account: account2, balance: -55, currency: "USD" } },
        ],
      };
    },
  },
  {
    description: "deposits in different currencies are tracked separately",
    run: () => {
      const account = randomUUID();
      return {
        given: [
          { messageType: "FundsDepositApproved", payload: { account, amount: 100, currency: "USD", transactionId: randomUUID() } },
          { messageType: "FundsDepositApproved", payload: { account, amount: 80, currency: "EUR", transactionId: randomUUID() } },
        ],
        then: [
          { key: `${account}:USD`, expectedState: { account, balance: 100, currency: "USD" } },
          { key: `${account}:EUR`, expectedState: { account, balance: 80, currency: "EUR" } },
        ],
      };
    },
  },
