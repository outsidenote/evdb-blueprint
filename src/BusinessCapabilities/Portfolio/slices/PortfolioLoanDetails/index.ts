...
export const portfolioLoanDetailsSlice: ProjectionConfig = {
  ...
  handlers: {
    // LoanRiskAssessed represents the full current state of a loan's risk assessment.
    // All fields are overwritten on conflict — no accumulation semantics apply here.
    LoanRiskAssessed: (payload, { projectionName }) => {
      const p = payload as PortfolioLoanDetailsPayload;
      const key = `${p.portfolioId}:${p.loanId}`;
      const acquisitionDate = p.acquisitionDate instanceof Date ? p.acquisitionDate.toISOString() : p.acquisitionDate;
