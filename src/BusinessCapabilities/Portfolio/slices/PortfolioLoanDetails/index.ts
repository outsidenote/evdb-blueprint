...
export const portfolioLoanDetailsSlice: ProjectionConfig = {
  ...
  handlers: {
    LoanRiskAssessed: (payload, { projectionName }) => {
      ...
      const acquisitionDateStr = p.acquisitionDate instanceof Date ? p.acquisitionDate.toISOString() : p.acquisitionDate;
      const maturityDateStr = p.maturityDate instanceof Date ? p.maturityDate.toISOString() : p.maturityDate;
      return [
        {
          sql: `
