...
- `slices/AddLoanToPortfolio/mcp.ts` — replace `description: "@DESCRIPTION_TODO"` with a 1-3 sentence MCP tool description. Verb-first. For command tools, mention the event types from the `emits` array. State when an agent should call this tool. Do NOT touch any other field in the descriptor.
...
## All scaffold output
  ...
  + src/BusinessCapabilities/Portfolio/slices/AddLoanToPortfolio/mcp.ts
