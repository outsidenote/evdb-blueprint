...
// Investment grade ratings: BBB and above. Anything below (CCC, B, BB, etc.) is sub-investment grade.
const INVESTMENT_GRADE_RATINGS = new Set(["AAA", "AA", "A", "BBB"]);
...
export const portfolioRatingBreached = (state: SliceStateAddLoanToPortfolioViewState, command: AddLoanToPortfolio): boolean =>
  state.portfolioId !== "" && !INVESTMENT_GRADE_RATINGS.has(command.creditRating);
...
export const portfolioRatingMaintained = (state: SliceStateAddLoanToPortfolioViewState, command: AddLoanToPortfolio): boolean =>
  INVESTMENT_GRADE_RATINGS.has(command.creditRating);
