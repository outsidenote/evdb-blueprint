 ...
 * @returns 
 */
export function createIdempotencyMessageFromEvent(event: EvDbEvent, idempotencyKey: string, consumerId: string): EvDbMessage {
    const key = getIdempotencyKey(idempotencyKey, consumerId);
    return EvDbMessage.createFromEvent(event, { payloadType: `${consumerId}.IdempotencyKeyAddedForConsumer`, idempotencyKey: key, consumerId }, "idempotent");
}
